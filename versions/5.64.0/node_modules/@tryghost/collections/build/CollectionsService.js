...
const TagDeletedEvent_1 = require("./events/TagDeletedEvent");
const debug = (0, debug_1.default)('collections');
const messages = {
...
class CollectionsService {
    ...
    subscribeToEvents() {
        ...
        this.DomainEvents.subscribe(PostAddedEvent_1.PostAddedEvent, async (event) => {
            ...
            try {
                ...
                logging_1.default.error({ err, message: 'Error handling PostAddedEvent' });
            }
        });
        this.DomainEvents.subscribe(PostEditedEvent_1.PostEditedEvent, async (event) => {
            ...
            try {
                ...
                logging_1.default.error({ err, message: 'Error handling PostEditedEvent' });
            }
        });
        ...
        //     }
        // });
        this.DomainEvents.subscribe(post_events_1.PostsBulkUnpublishedEvent, async (event) => {
            logging_1.default.info(`PostsBulkUnpublishedEvent received, updating collection posts ${event.data}`);
            try {
                await this.updateUnpublishedPosts(event.data);
                /* c8 ignore next 3 */
            }
            catch (err) {
                logging_1.default.error({ err, message: 'Error handling PostsBulkUnpublishedEvent' });
            }
        });
        this.DomainEvents.subscribe(post_events_1.PostsBulkFeaturedEvent, async (event) => {
            logging_1.default.info(`PostsBulkFeaturedEvent received, updating collection posts ${event.data}`);
            try {
                await this.updateFeaturedPosts(event.data);
                /* c8 ignore next 3 */
            }
            catch (err) {
                logging_1.default.error({ err, message: 'Error handling PostsBulkFeaturedEvent' });
            }
        });
        this.DomainEvents.subscribe(post_events_1.PostsBulkUnfeaturedEvent, async (event) => {
            logging_1.default.info(`PostsBulkUnfeaturedEvent received, updating collection posts ${event.data}`);
            try {
                await this.updateFeaturedPosts(event.data);
                /* c8 ignore next 3 */
            }
            catch (err) {
                logging_1.default.error({ err, message: 'Error handling PostsBulkUnfeaturedEvent' });
            }
        });
        this.DomainEvents.subscribe(TagDeletedEvent_1.TagDeletedEvent, async (event) => {
            logging_1.default.info(`TagDeletedEvent received for ${event.data.id}, updating all collections`);
            try {
                await this.updateAllAutomaticCollections();
                /* c8 ignore next 3 */
            }
            catch (err) {
                logging_1.default.error({ err, message: 'Error handling TagDeletedEvent' });
            }
        });
        this.DomainEvents.subscribe(post_events_1.PostsBulkAddTagsEvent, async (event) => {
            logging_1.default.info(`PostsBulkAddTagsEvent received for ${event.data}, updating all collections`);
            try {
                await this.updateAllAutomaticCollections();
                /* c8 ignore next 3 */
            }
            catch (err) {
                logging_1.default.error({ err, message: 'Error handling PostsBulkAddTagsEvent' });
            }
        });
    }
    hasPostEditRelevantChanges(postEditEvent) {
        const current = {
        ...
        return !(0, isEqual_1.default)(current, previous);
    }
    async updateAllAutomaticCollections() {
    ...
    async removePostsFromAllCollections(postIds) {
        return await this.collectionsRepository.createTransaction(async (transaction) => {
            const collections = await this.collectionsRepository.getAll({ transaction });
            for (const collection of collections) {
    ...
    async updatePostInMatchingCollections(postEdit) {
        return await this.collectionsRepository.createTransaction(async (transaction) => {
            ...
            if (collectionsChangeLog.length > 0) {
                logging_1.default.info(collectionsChangeLog);
            }
        });
    }
    async updateUnpublishedPosts(postIds) {
        return await this.collectionsRepository.createTransaction(async (transaction) => {
    ...
    async updateFeaturedPosts(postIds) {
        return await this.collectionsRepository.createTransaction(async (transaction) => {
            let collections = await this.collectionsRepository.getAll({
                filter: 'type:automatic+slug:-latest',
                transaction
    ...
    async updatePostsInCollections(postIds, collections, transaction) {
        const posts = await this.postsRepository.getAll({
